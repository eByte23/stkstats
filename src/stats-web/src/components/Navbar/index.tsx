...
const NavBarLink = ({ href, linkText }: { href: string, linkText: string }) => (
    <a
            ...
            <header className="flex flex-wrap sm:justify-start sm:flex-nowrap z-50 w-full bg-white text-sm py-3 sm:py-0 dark:bg-slate-900">
                <nav className="relative max-w-7xl w-full mx-auto px-4 sm:flex sm:items-center sm:justify-between sm:px-6 lg:px-8" aria-label="Global">
                    <div className="flex items-center justify-between">
                        <Link className="flex-none text-xl font-semibold dark:text-white" href="/" aria-label="St Kilda Stats">
                            <div className="flex">
                                <Image
                                    className="flex w-[50px]"
                                    src={"/stk-logo.png"} width={256} height={256} alt="STK Logo" />
                                <div className="flex pl-3 items-center">Stats</div>
                            </div>
                        </Link>
                    ...
                    <div id="navbar-collapse-with-animation" className="hs-collapse hidden overflow-hidden transition-all duration-300 basis-full grow sm:block">
                        <div className="flex flex-col gap-y-4 gap-x-0 mt-5 sm:flex-row sm:items-center sm:justify-end sm:gap-y-0 sm:gap-x-7 sm:mt-0 sm:pl-7">
                            ...
                            {/* <a className="flex items-center gap-x-2 font-semibold text-gray-500 hover:text-blue-600 sm:border-l sm:border-gray-300 sm:my-6 sm:pl-6 dark:border-gray-700 dark:text-gray-400 dark:hover:text-blue-500" href="#">
    ...
    <nav className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="relative flex flex-row justify-between items-center gap-x-8 border-t py-4 sm:py-0 dark:border-slate-700">
            <div className="flex items-center w-full sm:w-[auto]">
                <span className="font-semibold whitespace-nowrap text-gray-800 border-r border-r-white/[.7] sm:border-transparent pr-4 mr-4 sm:py-3.5 dark:text-white">My project</span>

                <div className="w-full sm:hidden">
                    <button type="button" className="hs-collapse-toggle group w-full inline-flex justify-between items-center gap-2 rounded-md font-medium text-gray-600 border border-gray-200 align-middle py-1.5 px-2 hover:text-gray-800 focus:outline-none focus:ring-2 focus:ring-white/[.5] transition" data-hs-collapse="#secondary-nav-toggle" aria-controls="secondary-nav-toggle" aria-label="Toggle navigation">
                        Overview
                        <svg className="hs-dropdown-open:rotate-180 w-2.5 h-2.5 text-gray-600 transition group-hover:text-gray-800" width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <div id="secondary-nav-toggle" className="hs-collapse hidden overflow-hidden transition-all duration-300 absolute top-16 right-0 w-full rounded-md bg-white sm:block sm:static sm:top-0 sm:w-full sm:max-h-full sm:bg-transparent sm:overflow-visible">
                <div className="flex flex-col py-2 sm:flex-row sm:justify-end sm:gap-y-0 sm:gap-x-6 sm:py-0">
                    <a className="text-sm font-semibold text-blue-600 py-2 sm:py-3.5 dark:text-white dark:hover:text-white" href="#">Overview</a>
                    <a className="text-sm font-medium text-gray-800 hover:text-blue-600 sm:border-b-2 border-b-transparent py-2 sm:py-3.5 dark:text-gray-400 dark:hover:text-white" href="#">Features</a>
                    <a className="text-sm font-medium text-gray-800 hover:text-blue-600 sm:border-b-2 border-b-transparent py-2 sm:py-3.5 dark:text-gray-400 dark:hover:text-white" href="#">Platforms</a>
                    <a className="text-sm font-medium text-gray-800 hover:text-blue-600 sm:border-b-2 border-b-transparent py-2 sm:py-3.5 dark:text-gray-400 dark:hover:text-white" href="#">Pricing</a>
                </div>
            </div>
        </div>
    </nav>)
